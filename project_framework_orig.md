...
my_project/
│
├── data/             # Step 1: Get and prepare your data
│   └── __init__.py
├── models/           # Step 2: Define your model architecture
│   └── __init__.py
...
```
data/ → models/ → train.py → eval.py → inference.py
   ↓          ↓           ↓          ↓           ↓
...
**Typical flow:**
1. Initialize model from `models/`
...
- **Workflow:** `data.py` → `model.py` → `train.py` → `eval.py`
...
3. **Define your model**
   - Create model class in `model.py`
